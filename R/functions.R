...
plot.influridge <- function(X, y, nw = 100, max.weight = 4, control.list = list(factr = 1e-4)){
  n <- dim(X)[1]
  ...
  matplot(weights, t(lambdaMatrix),
...
tuning.cv.svd <- function(lambda, w, svd.int,y.int) {
